...
export const AddCourseForm = ({ setCourseData, program, period }: Props) => {
  ...
  useEffect(() => {
    if (avatar) {
      setCourse({ ...course, ['image']: avatar.file });
    }
  }, [avatar]);
  ...
  const onChange = (event: ChangeEvent<HTMLInputElement>) => {
    setCourse({ ...course, programa: program, periodo: period, [event.target.name]: event.target.value });
  };
  ...
  return (
    <>
      ...
      <div className="container column">
        ...
        />
        <Spacer h={0.5} />
        <Input placeholder="Docente" name="nDocente" width="100%" clearable onChange={(event) => onChange(event)} />
        <Spacer h={0.5} />
      </div>
